...
For an existing Elasticsearch installation, first upgrade every application and continuous-sync worker to the version that excludes tool and blank messages. Then preview the ineligible PostgreSQL source count with `bun run fts-search:cleanup-ineligible-messages --status` and separately record the Elasticsearch `messages` alias's live `role=tool` count. When scheduling the separate cleanup, run bounded batches with `--apply --yes --all-workers-upgraded --state-file=/private/path/ineligible-messages.json --batch-size=250 --max-batches=10 --delay-ms=1000`. The state file stores the source cursor privately; keep it for resume and do not delete it between runs. A repeated batch is safe: it allocates a newer Outbox revision, and the normal sync worker writes a fenced soft tombstone or the message's current eligible projection. Wait for the Outbox to drain without dead letters, then confirm the Elasticsearch count has decreased toward zero. The command does not change PostgreSQL messages or run Elasticsearch writes itself.
