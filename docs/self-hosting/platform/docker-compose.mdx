...
<Callout type="warning">
  **Upgrading an existing Docker Compose deployment?** The latest `docker-compose.yml` adds a gateway service for Gateway Mode and removes the `rustfs-init` container. Update `.env` as described in [Upgrading an Existing Deployment](#upgrading-an-existing-deployment) before starting it. Otherwise the gateway container keeps restarting, agent runs fall back to the browser, and `docker logs lobehub` prints an `ACTION REQUIRED` warning.
</Callout>
...
<Steps>
  ...
  `--remove-orphans` removes the old `rustfs-init` container: LobeHub now creates the storage bucket on startup.
</Steps>
...
LobeHub creates the bucket on startup; if that fails, `docker compose logs lobe` contains `Failed to ensure the S3 bucket exists`.
