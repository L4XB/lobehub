...
<Callout type="warning">
  **正在升级已有的 Docker Compose 部署？** 最新的 `docker-compose.yml` 新增了 Gateway 模式所需的网关服务，并移除了 `rustfs-init` 容器。启动前请先按 [升级已有部署](#升级已有部署) 更新 `.env`，否则网关容器会反复重启、智能体退回浏览器中运行，`docker logs lobehub` 也会打印 `ACTION REQUIRED` 告警。
</Callout>
...
<Steps>
  ...
  `--remove-orphans` 会清理旧的 `rustfs-init` 容器：存储桶现在由 LobeHub 在启动时自动创建。
</Steps>
...
存储桶由 LobeHub 在启动时自动创建；如果创建失败，`docker compose logs lobe` 中会出现 `Failed to ensure the S3 bucket exists`。
